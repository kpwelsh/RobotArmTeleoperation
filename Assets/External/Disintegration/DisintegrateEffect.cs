...
namespace A5BGames.DisintegrationEffect
{
    public class DisintegrateEffect : MonoBehaviour
    {
        ...
        public static DisintegrateEffect Create (Vector3 position, Quaternion rotation, Vector3 scale) {
            // get the GameObject
        ...
        private void RenderEffect () {
            ...
            var cameraRotation = Quaternion.identity;// mainCamera.transform.rotation;

            // locally store object position and rotation
            var objectPosition = transform.position;
            objectPosition.Scale(transform.localScale);
            ...
            for (int i = activeParticles.Count - 1; i >= 0; i--) {
                ...
                if (isDelayOver) {
                    var matrix = Matrix4x4.TRS (position, cameraRotation, objectScale * (1 - scale));
                    renderMatrices.Add (matrix);
                }
                else {
                    var matrix = Matrix4x4.TRS (position, objectRotation, objectScale);
